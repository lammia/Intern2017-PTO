...
@section('title')
@parent Welcome Manager
@stop

@section('content')
<?php 
$id = Session::get('session_emailmanager'); //echo $id;
$res = DB::select('select * from managers where email = :email', ['email' => $id]); //print_r( $res);
$user = DB::table('managers')->where('mgid', $res[0]->mgid)->pluck('teamid'); //print_r( $user);
...
$now = date('Y-m-d',time());
$now = strtotime($now);
$date = DB::table('pto_request')->where('mgid', $res[0]->mgid)->pluck('rqid','datestart');
$start = null;
$cloneID=null;
